...
class AldecRunner(SimRunner):
    ...
    def _compile_library(self, library, force_compile=False) -> 'HDLLibrary':
        ...
        if not os.path.isdir(library_compile_path):
            self._run_cmd(command=[vlib_exec, library.get_name()],
                          path=libraries_path)

        # Map library
        self._run_cmd(command=[vmap_exec, library.get_name(), library_compile_path],
                      path=libraries_path)
    ...
    def _get_netlist_call(self) -> str:
        ...
        netlist_obj = [obj for lib in lib_cont.get() for obj in lib.get_hdlfile_list() if obj.get_is_netlist()]
        call = ''
    ...
    def _write_run_do_file(self, test, generic_call, module_call):
        ...
        '''
        sim_call = self._get_simulator_call(generic_call, module_call)
    ...
    def _simulate(self, test, generic_call, module_call) -> bool:
        ...
        success = self._run_cmd(command=command,
