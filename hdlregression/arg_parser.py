...
def arg_parser_reader(arg_parser=None):
    ...
    # Python 3
        ...
        arg_parser.add_argument('-fr', '--fullRegression', action='store_true', help='run full regression')
        arg_parser.add_argument('-c', '--clean', action='store_true', help='clean before regression run')
        arg_parser.add_argument('-tc', '--testCase', action='store', type=str, nargs=1, help='run selected testbench[.architecture[.testcase]]')
        arg_parser.add_argument('-tg', '--testGroup', action='store', type=str, nargs=1, help="run selected testgroup(s)")
        arg_parser.add_argument('-ltc', '--listTestcase', action='store_true', help='list testcases')
        arg_parser.add_argument('-ltg', '--listTestgroup', action='store_true', help='list testgroups')
        arg_parser.add_argument('-lco', '--listCompileOrder', action='store_true', help='list compile order')
        arg_parser.add_argument('-fc', '--forceCompile', action='store_true', help='force recompile')
...
def arg_parser_update_settings(settings, args) -> 'HDLRegressionSettings':
    ...
    # ----------------------------------------------
    if args.simulator:
        settings.set_simulator_name(simulator_name=args.simulator[0], cli_selected=True)
