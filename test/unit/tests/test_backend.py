...
def test_test_folder(sim_env, tb_path):
    clear_output()
...
def test_new_test_file_added_in_2nd_run(sim_env, tb_path):
    clear_output()
    hr = HDLRegression(simulator=sim_env["simulator"])

    # 1st run, one file
    hr.add_files(get_file_path(tb_path + "/tb_passing.vhd"), "test_lib")
    hr.set_result_check_string("passing testcase")
    hr.start()

    # 2nd run, new file - old file no longer part of run
    hr = HDLRegression(simulator=sim_env["simulator"])
    hr.add_files(get_file_path(tb_path + "/tb_new_passing.vhd"), "test_lib_2")
    hr.set_result_check_string("passing testcase")
    hr.start()
...
def test_old_test_run_in_test_backup(sim_env, tb_path):
    clear_output()
    hr = HDLRegression(simulator=sim_env["simulator"])
    test_files = get_file_path(tb_path + "/tb_passing.vhd")
    hr.add_files(test_files, "test_lib")
    ...
    # 2nd run
    hr.add_files(get_file_path(tb_path + "/tb_passing_2.vhd"), "test_lib")
    hr.start()
...
def test_new_test_run_not_in_test_backup(sim_env, tb_path):
    clear_output()
    hr = HDLRegression(simulator=sim_env["simulator"])
    test_files = get_file_path(tb_path + "/tb_passing.vhd")
    hr.add_files(test_files, "test_lib")
...
def test_keep_code_coverage_preserves_test_results(sim_env, tb_path):
    clear_output()
    hr = HDLRegression(simulator=sim_env["simulator"])
    test_files = get_file_path(tb_path + "/tb_passing.vhd")
    hr.add_files(test_files, "test_lib")
    ...
    # 2nd run
    hr = HDLRegression(simulator=sim_env["simulator"])
    hr.add_files(get_file_path(tb_path + "/tb_passing_2.vhd"), "test_lib_2")
    hr.set_result_check_string("passing testcase")
