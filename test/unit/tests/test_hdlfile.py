...
def test_get_filename(sim_env, tb_path):
    hdlregression = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/my_tb_rand.vhd")
    hdlFile = HDLFile(
    ...
    exp_name = get_file_path(tb_path + "/my_tb_rand.vhd")
    assert exp_name in filename, "Expecting init file name"


def test_change_filename(sim_env, tb_path):
    hdlregression = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/my_tb_rand.vhd")
    hdlFile = HDLFile(
        filename_with_path=filename,
    ...
    exp_filename = get_file_path(tb_path + "/tb_testcase.vhd")
...
def test_file_type(sim_env, tb_path):
    hr = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/tb_testcase.vhd")
...
def test_remove(sim_env, tb_path):
    hr = HDLRegression(simulator=sim_env["simulator"])
...
def test_get_library(sim_env, tb_path):
    hdlregression = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/my_tb_rand.vhd")
    hdlFile = HDLFile(
        filename_with_path=filename,
...
def test_change_library(sim_env, tb_path):
    hdlregression = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/my_tb_rand.vhd")
    hdlFile = HDLFile(
        filename_with_path=filename,
...
def test_netlist_detection_with_vhd_file(sim_env, tb_path):
    hr = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/tb_testcase.vhd")
    hr.add_files(filename, "filetype_lib")
    hr.start()
...
def test_netlist_detection_with_sdf_file(sim_env, tb_path):
    hr = HDLRegression(simulator=sim_env["simulator"])
    filename = get_file_path(tb_path + "/netlist.sdf")
    hr.add_files(filename, "filetype_lib")
    hr.start()
...
def test_add_system_verilog_file_filename_accepted(sim_env, tb_path):
    """
    ...
    test_files = get_file_path(tb_path + "/dummy_sv_file.sv")
...
def test_add_system_verilog_file_filetype(sim_env, tb_path):
    """
    Test adding SV files.
    """
